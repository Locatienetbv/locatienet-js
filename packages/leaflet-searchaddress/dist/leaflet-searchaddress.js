...
(function (global, factory) {
    ...
    class CountriesDropDown {
        ...
        async _loadCountries() {
            try {
                this.countries = await apiClient_minExports.Api.countries();
                this._render();
            }
            catch (err) {
    ...
    `;
        ...
        _attachClickHandler() {
            this.container.addEventListener('click', (e) => {
                const target = e.target.closest('a[data-country]');
                if (!target)
                    return;
                e.preventDefault();
    ...
    */
    ...
    `;
        ...
        _bindEvents() {
            // Key events
            this.queryInput.addEventListener('keyup', (e) => {
                const key = e.which || e.keyCode;
                const active = this.resultContainer.querySelector('.address-item.active');
                ...
                if ([16, 36].includes(key))
                    return;
                this.resultContainer.style.display = 'none';
                if (key === 13)
                    this._onEnterKeyPressed();
            ...
            this.resultContainer.addEventListener('click', (e) => {
                const target = e.target.closest('.address-item');
                if (!target)
                    return;
                e.preventDefault();
        ...
        _search() {
            ...
            this.token = window.setTimeout(() => {
                apiClient_minExports.Api.locateByText(query, this.options.selectedCountry || 'NL', { minimalResultScore: 0 }).then((data) => this._fill(data));
            }, 100);
        }
        _selectAddress(address) {
            if (typeof address === 'string')
                address = new window.Address(address);
            this.resultContainer.style.display = 'none';
    ...
    class SearchPositionControl extends L.Control {
        constructor(options = {}) {
        ...
        onAdd(map) {
            ...
            // Button
            this.button = L.DomUtil.create('a', 'd-flex justify-content-center align-items-center', container);
            this.button.href = '#';
            this.button.title = 'Enable reverse geocode';
            this.button.style.width = '30px';
            this.button.style.height = '30px';
            // Icon (SVG)
            this.icon = L.DomUtil.create('img', '', this.button);
        ...
        async reverseGeocode(lat, lng) {
            try {
                const results = await apiClient_minExports.Api.locateByPosition({ x: lng, y: lat });
                if (results.length > 0)
                    this._handleResult(results[0], lat, lng);
            }
            catch (err) {
                this.map.fire('searchposition:error', { error: err });
            }
        }
        _handleResult(result, lat, lng) {
            if (this.inputElement)
                this.inputElement.value = result.properties.description || '';
